    ...
    keys.forEach(id => {
        ...
        row.innerHTML = `
            <div class="job-info-text" onclick="loadSelectedJob('${id}')">
                💼 [${sName}] Batch: ${job.batchNumber} | Target: ${job.batchSize.toLocaleString()} Pcs (Created: ${job.createdTime || 'Unknown'})
            </div>
...
function createNewJobConfiguration() {
    ...
    let initialTotal = (initialBoxes * pBx) + initialLoose;

    jobsDatabase[id] = {
        id: id, createdTime: timestamp, stationId: stId, batchNumber: bNum,
        batchSize: bSz, prodPerBox: pBx, boxesPerLayer: bLy, layersPerPallet: lPl, startingProducts: sPr,
        customPallets: customArr, customPos: customPos, 
        previousTotalProducts: initialTotal, lastCalculatedBoxes: initialBoxes, lastCalculatedLoose: initialLoose,
        currentBoxesVal: initialBoxes > 0 ? initialBoxes : "", currentLooseVal: initialLoose,
        initialJobBoxesLock: initialBoxes, lastSyncedTotal: initialTotal,
        timer1State: TIME_30_MIN, timer2State: TIME_1_HOUR, 
        logs: [{ stamp: `[${timestamp.split(' - ')[0]}]`, text: `New Job Created on ${sName} (${getCurrentShift()}).`, type: `check` }]
    };

    document.getElementById('setupBatchNumber').value = ""; 
    document.getElementById('setupCustomPallets').value = ""; 
...
function loadSelectedJob(id) {
    ...
    previousTotalProducts = job.previousTotalProducts || 0; 
    lastCalculatedBoxes = job.lastCalculatedBoxes;
    lastCalculatedLoose = job.lastCalculatedLoose; 
    initialJobBoxesLock = job.initialJobBoxesLock || 0;
    savedLogsArray = [...job.logs];

    productsPerPallet = boxesPerLayer * layersPerPallet * prodPerBox;
    
    let customArr = job.customPallets || [];
    let customPos = job.customPos || "end";
    let sumCustom = customArr.reduce((a, b) => a + b, 0);
    let standardPcs = Math.max(0, batchSize - sumCustom);
    
    let sequence = [];
    while (standardPcs > 0) {
        if (standardPcs >= productsPerPallet) { sequence.push(productsPerPallet); standardPcs -= productsPerPallet; } 
        else { sequence.push(standardPcs); standardPcs = 0; }
    }
    
    if (customPos === "start") job.palletSequence = customArr.concat(sequence);
    else job.palletSequence = sequence.concat(customArr);
    ...
    let exactBoxes = (batchSize / prodPerBox).toFixed(1).replace('.0', '');
    ...
    let initialCalculatedTotal = job.lastSyncedTotal || 0;
    ...
    isRunning = false; updateDisplays(); 
...
function deleteJobFromDatabase(id, event) {
    ...
    if (confirm("Are you sure you want to delete this job permanently?")) {
        if (activeJobId === id) { exitToJobList(); }
...
function updatePalletFills(currentTotal) {
    ...
    for (let i = 0; i < seq.length; i++) {
        if (tempTotal >= seq[i]) { completedPallets++; tempTotal -= seq[i]; } 
        else { activePalletSize = seq[i]; break; }
    }
    ...
    if (currentTotal >= batchSize) {
        ...
        activeIcon.className = "pallet-icon full"; activeIcon.style.setProperty('--fill-level', '100%');
        pctText.textContent = "100%"; pctText.style.color = "#a6e3a1";
        boxRemaining.style.borderColor = "#a6e3a1"; valRemaining.style.color = "#a6e3a1";
        if (extra > 0) { valRemaining.style.fontSize = "1.2rem"; valRemaining.innerHTML = `DONE<br><span style="font-size:0.9rem; color:#f9e2af;">+${extra} EXTRA</span>`; } 
        else { valRemaining.style.fontSize = "1.8rem"; valRemaining.textContent = "DONE"; }
    } else {
        activeIcon.className = "pallet-icon"; activeIcon.style.setProperty('--fill-level', `${pct}%`);
        pctText.textContent = `${pct}%`; pctText.style.color = "#fab387";
...
function updateShiftGoalUI(currentShiftProducts) {
    document.getElementById('displayShiftCurrent').textContent = currentShiftProducts.toLocaleString();
    document.getElementById('displayShiftTarget').textContent = globalShiftTarget.toLocaleString();
    let pct = 0;
    if(globalShiftTarget > 0) pct = Math.min(100, Math.floor((currentShiftProducts / globalShiftTarget) * 100));
    const bar = document.getElementById('shiftProgressBar');
    bar.style.width = `${pct}%`; bar.textContent = `${pct}%`;
    bar.style.backgroundColor = pct >= 100 ? "#a6e3a1" : "#89b4fa";
}
...
function initChart() {
    ...
    chartInstance = new Chart(ctx, {
        type: 'bar',
        data: { labels: globalData.chartLabels, datasets: [{ label: 'Hourly Units', data: globalData.chartData, backgroundColor: '#fab387', borderColor: '#fab387', borderWidth: 1 }] },
...
function checkBoxesChange() {
    ...
    if (lastCalculatedBoxes !== null && currentBoxes > lastCalculatedBoxes) document.getElementById('currentLoose').value = "0";
}
    ...
    try {
        ...
        if(data[stationId]) {
            ...
            if (data[stationId].reset_flag === true) {
                ...
                if(activeJobId) {
                    document.getElementById('resShiftTotal').textContent = "0";
                    updateShiftGoalUI(0);
                    if (chartInstance) {
...
function acknowledgeSupervisorMessage() {
...
function playAckVoice() {
    let wasRadio = !radioAudio.paused;
    stopRadio();
    window.speechSynthesis.cancel();
    ...
    try {
        ...
        stations.forEach(station => {
            if (station.url_resolved && station.name.trim() !== "") {
                let opt = document.createElement('option'); opt.value = station.url_resolved; opt.textContent = station.name.trim();
...
function addLogEntry(text, typeClass) {
    ...
    let now = new Date();
    let stamp = `[${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}:${String(now.getSeconds()).padStart(2,'0')}]`;
